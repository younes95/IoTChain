...
var onmessage = function(payload) {
    ...
    if(message.type == 8){
        ...
        console.log(minerTurn(fileMiner));
    ...
    if(message.type == 11){
        ...
        mac = message.mac;
        //Update access list
        update_access_list(toHexString(publicKey),mac,fileAccess);
        //Update adresses list
        update_adresses(toHexString(publicKey),mac,fileAdresses);
        data = get_node_info_by_adr(toHexString(publicKey),fileAdresses);
        ...
        if(data.table[0].role == 'miner'){
            ...
            if(dataMiner.length != 0 ){
                ...
                objMiner.table[0].tabAdr.push(toHexString(publicKey));
    ...
    if(dataAdresses.length != 0 ){
        ...
        for(i=0;i<Object.keys(objAdresses.table).length;i++){
            ...
            if(objAdresses.table[i].Node.role == 'miner'){
                ...
                var publicKey = new Buffer(nodeInfo.Key.publicKey,'hex');
                ...
                var isValid = ec.verify(shaMsg, mySign, publicKey)
                ...
                var packet = {
                    ...
                    message: { type: 11, host: nodeInfo.Server.IP, port: nodeInfo.Server.port, publicKey: publicKey, mac : macadr, signature : signature, shaMsg : shaMsg } 
                };
                server.sendMessage({address: objAdresses.table[i].Node.IP, port: objAdresses.table[i].Node.port},packet);
            }
        }
